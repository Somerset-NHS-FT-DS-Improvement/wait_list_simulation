...
def test_patient_categoriser_basic(sample_fcst_additions, sample_historic_data):
    ...
    assert (
        "cat" in output_df.columns
    ), "'cat' column should exist in the output DataFrame"

    # Check if the output index matches the forecast addition dates
    assert all(
        output_df.index == sample_fcst_additions.index
    ), "Index should match forecast addition dates"


def test_patient_categoriser_with_missing_data(
    sample_fcst_additions, sample_historic_data
):
    ...
    ), "Output should be a pandas DataFrame even with NaN values in historic data"
    assert len(output_df) == len(
        sample_fcst_additions
    ), "Output length should match the forecast addition length"
...
def test_patient_categoriser_probabilities(sample_historic_data):
    ...
    assert (
        sum(category_distribution.values()) == 10
    ), "The total count should match the forecast additions"


def test_patient_categoriser_random_seed_consistency(
    sample_fcst_additions, sample_historic_data
):
